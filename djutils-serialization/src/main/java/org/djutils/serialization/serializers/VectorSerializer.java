...
/**
 * (De)serializes a DJUNITS Vector.
 * <p>
 ...
 */
public class VectorSerializer<U extends Unit<U, Q>, Q extends Quantity<Q>, V extends Vector<Q, V, ?, ?, ?>>
        extends ArrayOrMatrixWithUnitSerializer<U, V>
{
    ...
    private static final Map<Unit<?, ?>, Constructor<? extends Vector<?, ?, ?, ?, ?>>> CACHE = new HashMap<>();
    ...
    @Override
    public void serialize(final V vector, final byte[] buffer, final Pointer pointer, final Endianness endianness)
    {
        endianness.encodeInt(vector.size(), buffer, pointer.getAndIncrement(4));
        encodeUnit((U) vector.getDisplayUnit(), buffer, pointer, endianness);
