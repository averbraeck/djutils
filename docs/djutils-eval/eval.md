...
```
100[m/s] + 10 [s]: Cannot add 100 m/s to 10 s because the types are incompatible at position 17
```
...
Expressions with plain (untyped) values cannot fail due to incompatible types. They can, of course, fail due to operands being invalid for the operation. In some cases this results in the special value NaN (Not-a-Number). E.g. `sqrt(-1)` yields the value _NaN_ without throwing an exception.

Other cases, like division by zero _do_ throw an exception:

```
1/0: Division by 0 at position 3
```
...
> Note that the `!` is not the factorial operation.
...
will yield a double value of approximately 0.8414709848 (angles are in radians). A zero-argument function is followed by an empty pair of parentheses. This distinguishes it from a variable which is not followed by an opening parenthesis.
...
```
CURRENTTIME()+5: Left operand of addition must be a scalar (got "1779388691 s") at position 15
```
