...
/**
 * A {@link ImmutableSortedSet} extended with navigation methods reporting closest matches for given search targets.
 * Methods {@code lower}, {@code floor}, {@code ceiling}, and {@code higher} return elements respectively less than,
 * less than or equal, greater than or equal, and greater than a given element, returning {@code null} if there is no
 * such element. All methods from java.util.NavigableSet that can change the set have been left out.
 * <p>
 * Copyright (c) 2016-2018 Delft University of Technology, Jaffalaan 5, 2628 BX Delft, the Netherlands. All rights reserved. See
...
public interface ImmutableNavigableSet<E> extends ImmutableSortedSet<E>
{
    ...
    /**
     * Returns the greatest element in this immutable set strictly less than the given element, or {@code null} if there
     * is no such element.
     * @param e E; the value to match
     * @return the greatest element less than {@code e}, or {@code null} if there is no such element
     * @throws ClassCastException if the specified element cannot be compared with the elements currently in the
     *             immutable set
     * @throws NullPointerException if the specified element is null and this immutable set does not permit null
     *             elements
     */
    E lower(E e);

    /**
     * Returns the greatest element in this immutable set less than or equal to the given element, or {@code null} if
    ...
    /**
     * Returns the least element in this immutable set greater than or equal to the given element, or {@code null} if
     * there is no such element.
     * @param e E; the value to match
    ...
    /**
     * Returns the least element in this immutable set strictly greater than the given element, or {@code null} if there
     * is no such element.
     * @param e E; the value to match
    ...
    /**
     * Returns an immutable iterator over the elements in this immutable set, in descending order. Equivalent in effect
     * to {@code descendingSet().iterator()}.
    ...
    /**
     * Returns a view of the portion of this immutable set whose elements range from {@code fromElement} to
     * {@code toElement}. If {@code fromElement} and {@code toElement} are equal, the returned immutable set is empty
     * unless {@code fromInclusive} and {@code toInclusive} are both true.
     * @param fromElement E; low endpoint of the returned immutable set
     * @param fromInclusive boolean; {@code true} if the low endpoint is to be included in the returned view
     * @param toElement E; high endpoint of the returned immutable set
     ...
     * @throws NullPointerException if {@code fromElement} or {@code toElement} is null and this immutable set does not
     *             permit null elements
     * @throws IllegalArgumentException if {@code fromElement} is greater than {@code toElement}; or if this immutable
     *             set itself has a restricted range, and {@code fromElement} or {@code toElement} lies outside the
     *             bounds of the range.
     */
     ...
     * @return a view of the portion of this immutable set whose elements are less than (or equal to, if
     *         {@code inclusive} is true) {@code toElement}
     * @throws ClassCastException if {@code toElement} is not compatible with this immutable set's comparator (or, if
     *             the immutable set has no comparator, if {@code toElement} does not implement {@link Comparable}).
     *             Implementations may, but are not required to, throw this exception if {@code toElement} cannot be
     ...
     * @throws IllegalArgumentException if this immutable set itself has a restricted range, and {@code toElement} lies
     ...
     *         {@code fromElement}
     * @throws ClassCastException if {@code fromElement} is not compatible with this immutable set's comparator (or, if
     *             the immutable set has no comparator, if {@code fromElement} does not implement {@link Comparable}).
     *             Implementations may, but are not required to, throw this exception if {@code fromElement} cannot be
     *             compared to elements currently in the immutable set.
     * @throws NullPointerException if {@code fromElement} is null and this immutable set does not permit null elements
     * @throws IllegalArgumentException if this immutable set itself has a restricted range, and {@code fromElement}
