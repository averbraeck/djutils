...
public interface ImmutableCollection<E> extends Iterable<E>, Serializable
{
    ...
    /**
     * Returns <tt>true</tt> if this immutable collection contains the specified element. More formally, returns
     * <tt>true</tt> if and only if this immutable collection contains at least one element <tt>e</tt> such that
     ...
     * @throws NullPointerException if the specified element is null and this immutable collection does not permit null
     *             elements
     */
    boolean contains(Object o);

    /**
     * Returns an immutable iterator over the elements in this immutable collection. There are no guarantees concerning
     * the order in which the elements are returned (unless this immutable collection is an instance of some class that
     * provides a guarantee). The ImmutableIterator is an Iterator where the remove() operation will throw an exception.
    ...
    /**
     * Returns an array containing all of the elements in this immutable collection. If this immutable collection makes
     * any guarantees as to what order its elements are returned by its iterator, this method must return the elements
     * in the same order.
     * <p>
    ...
    /**
     * Returns an array containing all of the elements in this immutable collection; the runtime type of the returned
     * array is that of the specified array. If the immutable collection fits in the specified array, it is returned
     ...
     * @param a the array into which the elements of this immutable collection are to be stored, if it is big enough;
     ...
     *             every element in this immutable collection
     ...
     * @throws NullPointerException if the specified collection contains one or more null elements and this immutable
    ...
    /**
     * Returns <tt>true</tt> if this immutable collection contains all of the elements in the specified immutable
     * collection.
     * @param c ImmutableCollection&lt;?&gt;; immutable collection to be checked for containment in this immutable
     *            collection
     * @return <tt>true</tt> if this immutable collection contains all of the elements in the specified immutable
     *         collection
     * @throws ClassCastException if the types of one or more elements in the specified immutable collection are
     *             incompatible with this immutable collection
     * @throws NullPointerException if the specified immutable collection contains one or more null elements and this
     *             immutable collection does not permit null elements, or if the specified immutable collection is null.
     * @see #contains(Object)
     */
    ...
    /**
     * Returns a possibly parallel {@code Stream} with this collection as its source. It is allowable for this method to
    ...
    /**
     * Return whether the internal storage is a wrapped pointer to the original collection. If true, this means that
     * anyone holding a pointer to this data structure can still change it. The users of the ImmutableCollection itself
     * can, however, not make any changes.
     * @return boolean; whether the internal storage is a wrapped pointer to the original collection
     */
    boolean isWrap();

    /**
     * Return whether the internal storage is a (shallow) copy of the original collection. If true, this means that
     * anyone holding a pointer to the original of the data structure can not change it anymore. Nor can the users of
     * the ImmutableCollection itself make any changes.
